import { FaRegFile, FaRegFilePdf, FaRegFileImage, FaRegFileWord, FaRegFileExcel, FaRegFileArchive, FaDownload, FaEye } from "react-icons/fa";
import { MdDelete } from "react-icons/md";
import DeleteButton from "./DeleteButton";

const DocumentCard = ({ document, adminId }) => {
  const { original_name, file_type, file_url, file_size, created_at, mime_type, email } = document;
  ...
  const getFileIcon = () => {
    const iconProps = { className: "text-4xl mb-3 transform group-hover:scale-110 transition-transform duration-300" };
    
    switch (fileType) {
      case 'image': return <FaRegFileImage {...iconProps} />;
      case 'pdf': return <FaRegFilePdf {...iconProps} />;
      case 'word': return <FaRegFileWord {...iconProps} />;
      case 'excel': return <FaRegFileExcel {...iconProps} />;
      case 'archive': return <FaRegFileArchive {...iconProps} />;
      default: return <FaRegFile {...iconProps} />;
    }
  };
  ...
  return (
    <div className="group relative">
      ...
      <div className="relative bg-white rounded-xl shadow-sm hover:shadow-xl transition-all duration-300 p-4 border border-gray-100 group-hover:border-blue-200">
        ...
        {/* Uploader Email */}
        {email && (
          <p className="text-xs text-gray-500 mb-3 truncate" title={`Uploaded by: ${email}`}>
        ...
        <div className="flex justify-between items-center pt-3 border-t border-gray-100">
          ...
          >
            {isPdf ? "View" : "Download"}
            {isPdf ? <FaEye className="w-4 h-4" /> : <FaDownload className="w-4 h-4" />}
          </a>
        </div>
      </div>
    </div>
